...
// ============================================================
// Mutation functions — used by simulation engine at runtime
// ============================================================

/** Runtime alerts injected by the simulation engine (not from seed data) */
const simulationAlerts: Alert[] = [];

/** Add a simulation-generated alert */
export function addSimulationAlert(alert: Alert): void {
  // Prevent duplicates
  if (simulationAlerts.some((a) => a.id === alert.id)) return;
  simulationAlerts.push(alert);
}

/** Get all alerts including simulation-generated ones */
export function getAllAlertsWithSimulation(): Alert[] {
  return [...alerts, ...simulationAlerts];
}

/** Get active alerts including simulation-generated ones */
export function getActiveAlertsWithSimulation(): Alert[] {
  return [...alerts, ...simulationAlerts].filter(
    (a) =>
      a.status === "new" ||
      a.status === "acknowledged" ||
      a.status === "investigating" ||
...
/** Get count of simulation-generated alerts (for badge) */
export function getSimulationAlertCount(): number {
  return simulationAlerts.length;
}

/** Clear simulation-generated alerts (on reset) */
export function clearSimulationAlerts(): void {
  simulationAlerts.length = 0;
}

/** Update a shipment's progress and position in-place */
export function updateShipmentProgress(
  shipmentId: string,
...
/** Increment alert count on a shipment */
export function incrementShipmentAlertCount(shipmentId: string): void {
  const s = shipments.find((sh) => sh.id === shipmentId);
  if (s) s.alertCount += 1;
}
