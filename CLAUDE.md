...
- **Session 6: GPS Simulation Engine — SKIPPED** — Built and reverted. Server-side simulation engine with SSE streaming, trigger points, and speed controls was implemented but removed. In production, real-time GPS data will come directly from the platform/device integration, making a client-side simulation engine unnecessary for the prototype.

- **Session 7: Shipment Lifecycle Pages** — Three fully functional shipment pages replacing placeholders.
  - **Shipment List** (`/shipments`): Professional data table (shadcn Table) with 8 columns (ID, Customer, Route, Status, Mode, Risk, ETA, Alerts). Column sorting (click headers), pagination (10/25/50 per page with Select), inline filters (customer/status/mode dropdowns + text search), CSV export button. Role-scoped: customer_admin/customer_user see only their customer data, customer dropdown hidden. Status pills with theme-aware colors, risk badges (1-10 scale, green/yellow/red). Clickable shipment IDs link to detail page.
  - **Shipment Detail** (`/shipments/[id]`): Header bar with ID, status pill, risk badge, priority, progress bar. 2-column layout: embedded Leaflet map (400px, dynamic import ssr:false) with planned route (blue dashed polyline), actual path (green solid from breadcrumbUtils), comparison toggle (red deviation markers with distance annotations via deviationUtils). Right column: info cards (Cargo, Driver/Vehicle, DDI checklist with check/x icons, Devices with battery bars). Full-width sections: Segment Timeline (horizontal visual bar + shadcn Accordion per segment with carrier/times/distance/reporting rate), Condition Charts (Recharts AreaChart for temperature + humidity with threshold ReferenceLines and breach ReferenceAreas), Alerts list (reused AlertsTab), Audit log (reused AuditTab). Share tracking dialog for OEM partner URL.
  - **Creation Wizard** (`/shipments/new`): 5-step wizard with step indicator (circles + connecting lines, G4S-red active, green completed, gray future). Step 1: Route selection grid cards from templates with risk score, distance, modes, country transits, high-risk warning. Step 2: Shipment details form (customer select, priority, cargo description/value/weight, driver/phone/plate, temp range, SMP version, DDI 8-checkbox checklist). Step 3: Segment config auto-generated from route waypoints with carrier/scheduled times/reporting rate inputs. Step 4: Device assignment cards with battery bar, signal bar, sensor dots (GPS/Temp/Hum/Shock/Light/Door), low battery (<20%) disabled. Step 5: Review summary + large green DISPATCH button → addShipment() → redirect to detail.
  - **Shared utilities**: `src/lib/shipment-utils.ts` (displayRiskScore 0-100→1-10, riskScoreColor/BgColor, MODE_ICONS, formatCurrency, formatETA). `store.ts` added addShipment() + getNextShipmentId().
  - **New components**: ShipmentTable, ShipmentDetailMap, SegmentTimeline, ConditionCharts, ShareTrackingDialog, wizard/ (WizardStepIndicator, Step1-5).
  - **Dependencies added**: recharts, shadcn table/select/accordion.
  - **Key architectural decisions**: Detail map dynamically imported (ssr:false). Breadcrumbs use numeric seed (hash of shipment ID). Condition charts use Recharts AreaChart with gradient fills. Wizard state in single useState object. New shipments added to in-memory store (reset on reload).
