...
- **Session 4: Map Console Advanced** — Right side panel (400px flex sibling) with 5 shadcn Tabs: Overview (status, cargo, driver, ETA, progress, risk, DDI), Route (waypoints table, distance/duration/countries, deviation count), Alerts (severity pills, SLA status), Devices (battery/signal bars, sensor dots), Audit (timeline with color-coded dots). Simulated GPS breadcrumbs: Haversine distance along planned route, cut at progressPercent, deterministic LCG+Box-Muller jitter (~300m). Deviation detection: min-distance to planned path, 500m threshold, consecutive-point clustering. GeofenceOverlays: no-go zones (red dashed polygons), safe parking (green 800m circles), customer sites (blue CircleMarkers). RiskHeatmap: leaflet.heat with 27 hardcoded corridor heat points (Serbian A1, N.Macedonia border, Greek-Bulgarian border, etc.), green→yellow→orange→red gradient. Filter bar extended with 4 geofence toggle buttons (No-Go, Safe Park, Sites, Heatmap) with colored active states. Marker click opens side panel + stops propagation. Map click closes panel. MapResizer invalidates map size on panel toggle. Popup replaced by lightweight Tooltip (shipment ID only). Legend dynamically adds entries for active overlays. Build passes clean.
